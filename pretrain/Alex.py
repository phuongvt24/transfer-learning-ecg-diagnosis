...
class DenseNet(torch.nn.Module):
    def __init__(self, layer_num=(6,12,24,16), growth_rate=32, init_features=64, in_channels=12, middele_channels=128, classes=5):
        super(DenseNet, self).__init__()
        self.feature_channel_num = init_features
        self.conv = torch.nn.Conv1d(in_channels, self.feature_channel_num, kernel_size=7, stride=2, padding=3) # Chỉnh sửa số lượng channels của lớp convolution đầu tiên
        ...
        self.classifier = torch.nn.Linear(self.feature_channel_num, classes)  # Chỉnh sửa lớp fully connected cuối cùng
