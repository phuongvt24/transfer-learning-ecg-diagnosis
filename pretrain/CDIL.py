...
class CDILClassifier(nn.Module):
    def __init__(self,
                 ...
                 num_channels=None,
                 kernel_size=3,
                 num_classes=5):  # Thêm tham số num_classes
        super(CDILClassifier, self).__init__()
        ...
        self.classifier = nn.Linear(embedding_size, num_classes)  # Thêm tầng phân loại

    def forward(self, x):
        y_conv = self.conv(x)
        mean_pool = torch.mean(y_conv, dim=2)
        y = self.linear(mean_pool)
        y = self.classifier(y)  # Thêm bước phân loại
        return y
